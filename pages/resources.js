...
/* ---------------- Image paths (must match /public filenames exactly) ---------------- */
const HERO_IMG = "/man-child-bike.jpg";   // was var(--hero-bg-url)
const PRICING_IMG = "/woman-window.jpg";  // was var(--pricing-bg-url)
...
const HeroSection = () => (
  <section className="relative h-[80vh] flex items-center justify-center text-white text-center overflow-hidden">
    {/* Fixed background layer (inline URL, not CSS var) */}
    <div
      className="fixed-bg full-viewport"
...
const PricingSection = ({ onBuyToolkit, onBuyFounders, onPickSingle, onContribMonthly, onContribNYOP }) => (
  <section id="pricing" className="relative py-20 md:py-24 text-center text-gray-800 overflow-hidden">
    ...
    <div
      ...
      aria-hidden="true"
    />
    {/* opaque container so text sits above fixed bg */}
